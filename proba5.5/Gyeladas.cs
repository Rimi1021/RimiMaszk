...
namespace proba5._5
{
    public partial class Gyeladas : UserControl
    {
        public Gyeladas()
        {
            InitializeComponent();
        ...
        public static string telephelyarudb = "keszletarugyor";
        ...
        private void textBox_Kosarba_TextChanged(object sender, EventArgs e)
        {
            for (int i = 0; i < SzerverData.MaszInfokOsszes.Count; i++)
            {
                if (SzerverData.MaszInfokOsszes[i].Barcode == textBox_Kosarba.Text)
                {
                    ...
                    vanilyenbarcode = true;
                    label_adottarubrutt.Text = Convert.ToString(Aruvisszavetel.Brutto(SzerverData.MaszInfokOsszes[i].Ar_db)) + " Ft";
                    if (SzerverData.MaszInfokOsszes[i].Akcio != 0)
                    {
